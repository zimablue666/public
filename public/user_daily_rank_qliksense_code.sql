### Скрипт используется для ежедневного обновления базы рангов клиентов в qlik sense - по сумме их оборота на каждый день за посл. 30 дней
### ---------------------------

SELECT
sub_u.id as user_id,
sub_u.dataReg as user_dateReg,
sub_calls.id order_id,
...
and year(sub_calls.startTime)>=2022
...
and year(sub_text_o.created_at)>=2022

union ALL
SELECT
sub_chat.user_id,
sub_u3.dataReg as user_dateReg,
sub_chat.id order_id,
...
and year(sub_chat.start_time)>=2022
